...
public class UserViewModel extends ViewModel {
  ...
  public void signInAutomatically(Activity activity) {
    ...
    if (currentState == null
        || currentState.getStatus() != SignInState.Status.CHECKING_SESSION) {
      return;
    }
    if (!requestInProgress.compareAndSet(false, true)) {
      return;
    }
    userService.signInAutomatically(activity).whenComplete(this::handleAutomaticResult);
  }
  ...
  public void signInInteractively(Activity activity) {
    ...
    userService.signInInteractively(activity).whenComplete(this::handleInteractiveResult);
  }
